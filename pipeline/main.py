...
🔧 설정 파일:
  - ./configs/credentials/scienceon_api_credentials.json  (ScienceON API 자격증명)
""")
...
def get_gemini_api_key() -> str:
    ...
    config_path = Path("./configs/credentials/gemini_api_credentials.json")
...
def get_openai_api_key() -> str:
    ...
    # 설정 파일에서 확인
    script_dir = Path(__file__).parent
    config_path = script_dir.parent / "configs" / "credentials" / "chatgpt_api_credentials.json"
    if config_path.exists():
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
...
def get_pubmed_credentials() -> tuple[str, str]:
    ...
    # 설정 파일에서 확인
    config_path = Path("./configs/credentials/pubmed_api_credentials.json")
    if config_path.exists():
        try:
