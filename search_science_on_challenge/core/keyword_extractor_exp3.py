"""
sota prompt (f1 0.72) + 키워드 순서 임의로 변경 -> f1: 0.44
중요도 순서로 배치하는것이 의미가 있다!
...
"""
    ...
    def generate_search_terms(self, keywords: Dict[str, List[str]]) -> List[str]:
        ...
        try:
            ...
            return search_terms[:10]  # 최대 10개 검색어
        ...
        if korean_kw and english_kw:
            for kr in korean_kw[:3]:  # 상위 3개
                for en in english_kw[:3]:  # 상위 3개
                    mixed_terms.append(f"{kr}|{en}")  # 한국어|영어
                    mixed_terms.append(f"{en}|{kr}")  # 영어|한국어 (순서 바꾼 버전)
        
        # 부분 조합 (더 긴 조합)
        if len(korean_kw) > 1:
            # 각 키워드가 맨 앞으로 오는 조합들. 중요 키워드 순서를 변경
            korean_rotations = self._generate_keyword_rotations(korean_kw)
            mixed_terms.extend(korean_rotations)
            
        
        if len(english_kw) > 1:
            # 각 키워드가 맨 앞으로 오는 조합들
            english_rotations = self._generate_keyword_rotations(english_kw)
            mixed_terms.extend(english_rotations)
