...
def _build_search_terms_by_lang(keywords: dict[str, list[str]]) -> dict[str, list[str]]:
    """Build per-language search-term lists, isolating each language."""
    ko_kw = list(keywords.get("korean") or [])
    en_kw = list(keywords.get("english") or [])
    return {
        "korean": build_search_terms(
            {"korean": ko_kw, "english": []}, number_of_operators=0
        ) if ko_kw else [],
        "english": build_search_terms(
            {"korean": [], "english": en_kw}, number_of_operators=0
        ) if en_kw else [],
    }


def _extract_query_terms(
    keyword_extractor: Any, query: str
) -> tuple[dict[str, Any], dict[str, list[str]]]:
    keywords = keyword_extractor.extract_keywords(query)
    return keywords, _build_search_terms_by_lang(keywords)


def _search_terms_until_target(
    client: Any, terms: list[str], target_documents: int, source_name: str, lang: str
) -> list[dict[str, Any]]:
    """Term-major search: iterate terms, accumulate + dedup, stop when target met."""
    ...
    for term in terms:
        ...
        try:
            ...
            logging.error(
                "%s search failed (lang=%s, term=%s): %s", source_name, lang, term, e
            )
            continue
        collected = remove_duplicates(
            collected + docs, key="doc_id", fallback_keys=("title",)
        )
...
def emit_frozen_queries(questions_path: str, output_path: str, keyword_extractor: Any) -> str:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        for row in _load_questions(Path(questions_path)):
            try:
                keywords, terms_by_lang = _extract_query_terms(keyword_extractor, row["query"])
            except Exception as e:
                logging.error("Keyword extraction failed for frozen query '%s': %s", row["query"], e)
                keywords = {"english": [row["query"]], "korean": []}
                terms_by_lang = {"korean": [], "english": [row["query"]]}
            payload = {
                "question_id": row["question_id"],
                "query": row["query"],
                "keywords": keywords,
                "search_terms": terms_by_lang["korean"] + terms_by_lang["english"],
...
def run_step1(
    ...
    for row in _load_questions(Path(questions_path)):
        ...
        if frozen:
            ...
            try:
                keywords, terms_by_lang = _extract_query_terms(keyword_extractor, query)
            except Exception as e:
                logging.error("Keyword extraction failed for query '%s': %s", query, e)
                keywords = {"english": [query], "korean": []}
                terms_by_lang = {"korean": [], "english": [query]}

        ko_terms = terms_by_lang.get("korean") or []
        en_terms = terms_by_lang.get("english") or []
        ko_docs = _search_terms_until_target(client, ko_terms, target_documents, source_name, "korean")
        en_docs = _search_terms_until_target(client, en_terms, target_documents, source_name, "english")
