"""exp2b sweep driver — subprocess per embedding model with VRAM/RAM monitor + abort."""
...
def run_case(case: dict, output: Path, corpus: str, queries: str, gold: str,
             vram_limit: int, ram_limit: int, max_runtime: int = 1200) -> dict:
    case_id = case["case_id"]
    case_dir = output / case_id
    case_dir.mkdir(parents=True, exist_ok=True)

    cmd = [
        PYTHON, str(PROJECT_ROOT / "experiments/exp2b_embedding_models/run_one_model.py"),
        "--model", case["model_name"],
        "--corpus", corpus,
        "--queries", queries,
        "--gold", gold,
        "--output", str(case_dir),
        "--batch-size", str(case.get("batch_size", 32)),
    ...
    print(f"  model={case['model_name']} bs={case.get('batch_size',32)}", flush=True)
    ...
    while True:
        ...
        if rc is not None:
            break
        rss, vram = _rss_mib(proc.pid), _vram_mib()
    ...
    if rj.exists():
        ...
        result = {"case_id": case_id, "model": case["model_name"], "status": "no_result"}
...
def main() -> None:
    ...
    ap.add_argument("--max-runtime-per-case", type=int, default=1200)
    args = ap.parse_args()
    ...
    summary = {
        "experiment": "exp2b_embedding_models",
